...
//         const [result2] = await connection.execute('select * from products where id = ?', [id]);
//         if (!result2 || result2.length === 0) return res.status(404).json({ result: 0, message: 'Không tìm thấy' })
//         res.status(200).json({ result: 1, data: result2[0] })   
//     } catch (err) {
//         if(err.isJoi){
//             return res.status(400).json({result:0,message: err.details[0].message})
//         }
//         res.status(500).json({ result: 0, message: 'Lỗi' })
//     }
// }
...
exports.getAllProducts = async (req, res) => {
    ...
    try {
        ...
        if (category_id) {
            ...
            countQuery += ' and category_id like ?';
        ...
        if (limit && page && !isNaN(page) && !isNaN(limit)) {
            ...
            return res.status(200).json({ data: result, pagination: { page: pageNum, limit: limitNum, totalPages, total_items: total } });
        }
...
exports.deleteProduct = async (req, res) => {
    ...
    try {
        const [row] =await connection.execute('select 1 from order_items where product_id = ? limit 1',[id]);
        if(row.length >0){
            return res.status(200).json({ result: 0, message: 'Không thể xóa vì sản phẩm đang nằm trong đơn hàng' })
        }
